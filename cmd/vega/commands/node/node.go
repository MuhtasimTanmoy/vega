...
func (n *Command) startBlockchainClients() error {
	...
	if len(n.conf.PrimaryEthereum.RPCEndpoint) < 1 && n.conf.Blockchain.ChainProvider == blockchain.ProviderNullChain {
		return nil
	}

	var err error
	n.l2Clients, err = ethclient.NewL2Clients(n.ctx, n.Log, n.conf.PrimaryEthereum)
	if err != nil {
		return fmt.Errorf("could not instantiate ethereum l2 clients: %w", err)
	}

	n.primaryEthClient, err = ethclient.Dial(n.ctx, n.conf.PrimaryEthereum)
	if err != nil {
		return fmt.Errorf("could not instantiate ethereum client: %w", err)
	}

	n.secondaryEthClient, err = ethclient.SecondaryDial(n.ctx, n.conf.SecondaryEthereum)
	if err != nil {
		return fmt.Errorf("could not instantiate secondary ethereum client: %w", err)
	}

	n.primaryEthConfirmations = ethclient.NewEthereumConfirmations(n.conf.PrimaryEthereum, n.primaryEthClient, nil)

	n.secondaryEthConfirmations = ethclient.NewEthereumConfirmations(n.conf.SecondaryEthereum, n.secondaryEthClient, nil)
