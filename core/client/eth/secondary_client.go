...
type SecondaryClient struct {
	ETHClient
	ethConfig *types.ArbitrumConfig
...
func SecondaryDial(ctx context.Context, cfg Config) (*SecondaryClient, error) {
	if len(cfg.SecondRPCEndpoint) <= 0 {
		return nil, errors.New("no secondary ethereum rpc endpoint configured")
	}

	ethClient, err := ethclient.DialContext(ctx, cfg.SecondRPCEndpoint)
	if err != nil {
...
func (c *SecondaryClient) UpdateEthereumConfig(ctx context.Context, ethConfig *types.ArbitrumConfig) error {
	if c == nil {
		return nil
	}
